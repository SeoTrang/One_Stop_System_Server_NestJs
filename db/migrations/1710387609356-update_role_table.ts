...
export class UpdateRoleTable1710387609356 implements MigrationInterface {
    name = 'UpdateRoleTable1710387609356'
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`officer\` ADD CONSTRAINT \`FK_23e73cad42caf33d2b38e968e4c\` FOREIGN KEY (\`departmentId\`) REFERENCES \`department\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`role\` CHANGE \`parent_id\` \`parent_id\` int NOT NULL`);
        await queryRunner.query(`ALTER TABLE \`role\` CHANGE \`icon\` \`icon\` varchar(255) NOT NULL`);
        await queryRunner.query(`ALTER TABLE \`role\` CHANGE \`routerLink\` \`routerLink\` varchar(255) NOT NULL`);
